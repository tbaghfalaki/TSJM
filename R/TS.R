...
TS <- function(formFixed, formRandom, formGroup, formSurv, nmark, K1 = K1, K2 = K2,
...
# Scaling Gauss-Kronrod/Legendre quadrature
  for(j in 1:K){
      ...
      Alpha0[k]<-  inprod(betaS[1:NbetasS],XS[k,1:NbetasS])+inprod(alpha[],LP1[k,])
      Alpha1[k]<- inprod(alpha[],LP2[k,])
      Alpha2[k]<- inprod(alpha[],LP3[k,])
      ...
      haz[k]<- ((h[1]*step(s[1]-Time[k]))+
                    ...
                    (h[5]*step(Time[k]-s[4])))*
        exp(inprod(betaS[1:NbetasS],XS[k,1:NbetasS])+inprod(alpha[],mu1[k,]))
  ...
  i.jags <- function() {
    list(betaS = rep(0, NbetasS), alpha = rep(0, nmark))
  }
  ...
  if (n.chains2 > 1) {
    names(sim1$mean$alpha) <-
      ...
      names(sim1$Rhat$alpha) <- paste0("Marker", 1:nmark)
    ...
    SM <- rbind(
      ...
      cbind(sim1$mean$alpha, sim1$sd$alpha, sim1$q2.5$alpha, sim1$q97.5$alpha, sim1$Rhat$alpha),
    ...
    TDsurvival <- list(S_model = SM)
  } else {
    names(sim1$mean$alpha) <-
      names(sim1$sd$alpha) <-
      names(sim1$q2.5$alpha) <-
      names(sim1$q97.5$alpha) <- paste0("Marker", 1:nmark)
    ...
    SM <- rbind(
      ...
      cbind(sim1$mean$alpha, sim1$sd$alpha, sim1$q2.5$alpha, sim1$q97.5$alpha),
